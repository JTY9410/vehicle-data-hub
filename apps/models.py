...
class Vehicle(db.Model):
    ...
    색상: Mapped[str | None] = mapped_column("색상", String(32))
    미션: Mapped[str | None] = mapped_column("미션", String(32))
    차종: Mapped[str | None] = mapped_column("차종", String(64))
    인승: Mapped[str | None] = mapped_column("인승", String(32))
    성능점검일: Mapped[datetime | None] = mapped_column("성능점검일", DateTime(timezone=True))
    옵션정보: Mapped[str | None] = mapped_column("옵션정보", Text)
    유용옵션: Mapped[str | None] = mapped_column("유용옵션", Text)
    진단정보: Mapped[str | None] = mapped_column("진단정보", Text)
...
class ApiKey(db.Model):
    ...
    key_secret: Mapped[str | None] = mapped_column(Text)
